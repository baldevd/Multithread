...
void highWatermarkTest() {
    ...
    // Should see 96 items in list
    print();

    // Adding a 97th item (Key 96) should make the program hang
    reference(96);
...
void lowWatermarkTest() {
    ...
    // Should see 32 items in list
    print();
    
    // Attempting to clean would remove all items, program should hang
    clean(0);
...
void anotherTest() {
    ...
    // Only touch even keys (refcount of even number keys is 2 now)
    print();

    clean(0);

    // Should only see even keys
    print();
    ...
    // Odd number keys added back (should see 64 items)
    print();
    ...
    // Only touch first 32 items (refcount of first 32 keys now 2)
    print();

    clean(0);

    // Should only see 32 items (keys 0 to 32)
    print();
